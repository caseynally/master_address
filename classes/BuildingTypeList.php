<?php
/**
 * A collection class for BuildingType objects
 *
 ...
 */
class BuildingTypeList extends ZendDbResultIterator
{
	/**
	 ...
	 */
	public function find($fields=null,$order='building_type_id',$limit=null,$groupBy=null)
	{
		$this->select->from('building_types_master');

		// Finding on fields from the building_types_master table is handled here
		if (count($fields)) {
		...
		// above foreach only handles fields from the building_types_master table.
	...
	{
		return new BuildingType($this->result[$key]);
	}
}
