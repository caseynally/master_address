<?php
/**
 * A collection class for LocationType objects
 *
 ...
 */
class LocationTypeList extends ZendDbResultIterator
{
	/**
	...
	public function find($fields=null,$order='id',$limit=null,$groupBy=null)
	{
		...
		// Finding on fields from the addr_location_types_master table is handled here
		if (count($fields)) {
		...
		// above foreach only handles fields from the addr_location_types_master table.
	...
	/**
	 * Hydrates all the LocationType objects from a database result set
	 *
	...
	{
		return new LocationType($this->result[$key]);
	}
}
