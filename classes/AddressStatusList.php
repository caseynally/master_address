<?php
/**
 * A collection class for AddressStatus objects
 *
 ...
 */
class AddressStatusList extends ZendDbResultIterator
{
	/**
	...
	{
		$this->select->from('mast_address_status_lookup');

		// Finding on fields from the mast_address_status_lookup table is handled here
		if (count($fields)) {
		...
		// above foreach only handles fields from the mast_address_status_lookup table.
	...
	/**
	 * Hydrates all the AddressStatus objects from a database result set
	 *
	...
	{
		return new AddressStatus($this->result[$key]);
	}
}
