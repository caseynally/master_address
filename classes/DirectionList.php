<?php
/**
 * A collection class for Direction objects
 *
 ...
 */
class DirectionList extends ZendDbResultIterator
{
	/**
	 ...
	 */
	public function find($fields=null,$order='direction_code',$limit=null,$groupBy=null)
	{
		$this->select->from('mast_street_direction_master');

		// Finding on fields from the mast_street_direction_master table is handled here
		if (count($fields)) {
		...
		// above foreach only handles fields from the mast_street_direction_master table.
	...
	/**
	 * Hydrates all the Direction objects from a database result set
	 *
	...
	{
		return new Direction($this->result[$key]);
	}
}
