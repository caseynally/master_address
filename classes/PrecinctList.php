<?php
/**
 * A collection class for Precinct objects
 *
 ...
 */
class PrecinctList extends ZendDbResultIterator
{
	/**
	 ...
	 */
	public function find($fields=null,$order='precinct',$limit=null,$groupBy=null)
	{
		...
		if (isset($fields['code'])) {
			$fields['precinct'] = $fields['code'];
			unset($fields['code']);
		}

		// Finding on fields from the voting_precincts table is handled here
		if (count($fields)) {
		...
		// above foreach only handles fields from the voting_precincts table.
	...
	{
		return new Precinct($this->result[$key]);
	}
}
