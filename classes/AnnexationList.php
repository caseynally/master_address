<?php
/**
 * A collection class for Annexation objects
 *
 ...
 */
class AnnexationList extends ZendDbResultIterator
{
	/**
	 ...
	 */
	public function find($fields=null,$order='ordinance_number',$limit=null,$groupBy=null)
	{
		$this->select->from('annexations');

		// Finding on fields from the annexations table is handled here
		if (count($fields)) {
		...
		// above foreach only handles fields from the annexations table.
	...
	/**
	 * Hydrates all the Annexation objects from a database result set
	 *
	...
	{
		return new Annexation($this->result[$key]);
	}
}
