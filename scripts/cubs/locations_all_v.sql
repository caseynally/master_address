...
as select
		...
		upper(addr.street_number) as street_number,addr.street_id,
		upper(addr.address_type) as address_type,
		upper(addr.tax_jurisdiction) as tax_jurisdiction,
		addr.jurisdiction_id, addr.gov_jur_id,
		addr.township_id,
		upper(addr.section) as section,
		upper(addr.quarter_section) as quarter_section,
		addr.subdivision_id, addr.plat_id, addr.plat_lot_number,
		upper(addr.street_address_2) as street_address_2,
		upper(addr.city) as city, upper(addr.state) as state, upper(addr.zip) as zip,
		upper(addr.zipplus4) as zipplus4,
		upper(addr.census_block_fips_code) as census_block_fips_code,
		addr.state_plane_x_coordinate, addr.state_plane_y_coordinate, addr.latitude,
		addr.longitude, addr.notes as address_notes,
		upper(TRIM(names.street_direction_code)) AS street_direction_code,
		upper(street.post_direction_suffix_code) as post_direction_suffix_code,
		street.town_id, street.status_code,
		upper(street.notes) as street_notes, upper(names.street_name) as street_name,
		upper(names.street_type_suffix_code) as street_type_suffix_code,
		upper(names.street_name_type) as street_name_type,
		names.effective_start_date, names.effective_end_date,
		names.notes as street_name_notes,
		upper(suds.sudtype) as subunit_type,
		upper(suds.street_subunit_identifier) as subunit,
		suds.notes as subunit_notes,
