create or replace PROCEDURE                      create_address (
	...
	BEGIN
		...
		-- Get a new street address ID
		errorblock:='F';
		SELECT eng.street_address_id_s.NEXTVAL INTO myStreetAddressID FROM DUAL;
		...
		INSERT INTO eng.mast_address_assignment_hist (location_id, action_date, action, contact_id, notes, street_address_id)
		 VALUES (myLocationID, SYSDATE, 'ASSIGN', nContactID, vAssignmentNotes, myStreetAddressID);
		...
		-- Create the status for the address
		errorblock := 'M';
		INSERT into eng.mast_address_status (status_code, start_date, street_address_id)
		 VALUES (1, SYSDATE, myStreetAddressID);

		-- Generate the status message

		vMessage:=vStreetNumber||' '||cStreet_direction||' '||vStreetName||' '||vStreetSuffix||' '||cPost_direction||' successfully added as Location ID '||myLocationID;

	COMMIT;

	EXCEPTION
		WHEN address_already_exists THEN
			vMessage:=vStreetNumber||' '||cStreet_direction||' '||vStreetName||' '||vStreetSuffix||' '||cPost_direction||' already exists';
